    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=128, reuse=False, layers=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            input = [tf.reshape(throughput_conv, [batch_size, -1]), tf.reshape(download_time_conv, [batch_size, -1]),
                     tf.reshape(chunk_size_conv, [batch_size, -1]), buffer_size_dense, last_bit_rate_dense, end_delay_dense]
...
class FeedForwardCustPolicy(FeedForwardPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            print('processed_x', self.processed_x)
            throughput = self.processed_x[:, :past_frame_num]
            throughput = tf.reshape(throughput, [-1, past_frame_num, 1])
            throughput_conv = activ(conv1d(throughput, scope='throughput_conv1d'))
            ...
            input = [tf.reshape(throughput_conv, [batch_size, 5 * n_hiddens]), tf.reshape(download_time_conv, [batch_size, 5 * n_hiddens]),
                     tf.reshape(chunk_size_conv, [batch_size, 1 * n_hiddens]), buffer_size_dense, last_bit_rate_dense, end_delay_dense]

            print('input', input)
            processed_x = tf.concat(input, axis=1)
